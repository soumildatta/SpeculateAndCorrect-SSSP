...
The basis of optiimzation for this new algorithm is the traversal of the graph in a generational manner such that no child is processed before its parents are processed. This reduces the number of unnecessary passes through the graph.
...
1. Casual users simply looking to run the SSSP algorithm on graphs more efficiently
...
If the changes you made are significant, you are most welcome to contribute your efforts to the repository. To make contributions, fork the repository, make your changes, and create a pull request explaining the changes made. Your contributions may be accepted if they meet the standards of the project and do not contain breaking changes.
